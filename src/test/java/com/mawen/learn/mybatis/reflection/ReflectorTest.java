...
class ReflectorTest {
	...
	void shouldNotGetClass() {
		Reflector reflector = reflectorFactory.findForClass(Section.class);
		assertFalse(reflector.hasGetter("class"));
	}
	...
	void shouldSettersWithUnrelatedArgTypesThrowException() {
		...
		assertThatExceptionOfType(ReflectionException.class)
				.isThrownBy(() -> ambiguousInvoker.invoke(new BeanClass(), param))
				.withMessageMatching("Ambiguous setters defined for property 'prop2' in class '" + BeanClass.class.getName().replace("$", "\\$")
	...
	void shouldTwoGettersForNonBooleanPropertyThrowException() throws Exception {
		...
		assertThatExceptionOfType(ReflectionException.class)
				.isThrownBy(() -> ambiguousInvoker.invoke(new BeanClass(), new Integer[]{1}))
				.withMessageContaining("Illegal overloaded getter method with ambiguous type for property 'prop2' in class '"
	...
	void shouldTwoGettersWithDifferentTypesThrowException() throws Exception {
		...
		Invoker ambiguousInvoker = reflector.getGetInvoker("prop2");
		assertThatExceptionOfType(ReflectionException.class)
				.isThrownBy(() -> ambiguousInvoker.invoke(new BeanClass(), null))
				.withMessageContaining("Illegal overloaded getter method with ambiguous type for property 'prop2' in class '"
						+ BeanClass.class.getName()
						+ "'. This breaks the JavaBeans specification and can cause unpredictable results.");
	}
	...
	void shouldIgnoreBestMatchSetterIfGetterIsAmbiguous() {
		...
		assertThatExceptionOfType(ReflectionException.class)
				.isThrownBy(() -> ambiguousInvoker.invoke(new Bean(), param))
				.withMessageMatching("Ambiguous setters defined for property 'bool' in class '" + Bean.class.getName().replace("$", "\\$")
